...
export const analyticsService = {
    ...
    async getBasicStats() {
        try {
            ...
            const cacheKey = cacheKeys.basicStats(user.uid);
    ...
    async getMonthlyProgress() {
        try {
            ...
            // Try to get from cache first
            const cacheKey = cacheKeys.monthlyProgress(user.uid);
            const cachedData = cacheUtils.get(cacheKey);
    ...
    async getRecentStudyTime() {
        try {
            ...
            // Try to get from cache first
            const cacheKey = cacheKeys.recentStudyTime(user.uid);
            const cachedData = cacheUtils.get(cacheKey);
    ...
    async refreshBasicStats() {
        ...
        const cacheKey = cacheKeys.basicStats(user.uid);
        const freshData = await this.getBasicStatsFromFirestore();
        cacheUtils.set(cacheKey, freshData);
        return freshData;
    },

    async refreshMonthlyProgress() {
        const user = getCurrentUser();
        if (!user) {
            throw new Error('User not authenticated');
        }
        ...
        const cacheKey = cacheKeys.monthlyProgress(user.uid);
        const freshData = await this.getMonthlyProgressFromFirestore();
        cacheUtils.set(cacheKey, freshData);
        return freshData;
    },

    async refreshRecentStudyTime() {
        const user = getCurrentUser();
        if (!user) {
            throw new Error('User not authenticated');
        }
        ...
        const cacheKey = cacheKeys.recentStudyTime(user.uid);
        const freshData = await this.getRecentStudyTimeFromFirestore();
        cacheUtils.set(cacheKey, freshData);
        return freshData;
    },
    ...
    async getUserProgress(userId) {
        try {
            ...
            const freshData = await this._getCombinedProgressData();
            cacheUtils.set(cacheKey, freshData);
            return freshData;
        } catch (error) {
            handleFirebaseError(error, 'getting user progress');
            throw error;
        }
    },

    async _getCombinedProgressData() {
        try {
            const [basicStats, monthlyProgress, studyTime] = await Promise.all([
                this.getBasicStatsFromFirestore(),
                this.getMonthlyProgressFromFirestore(),
                this.getRecentStudyTimeFromFirestore()
            ]);
            ...
            handleFirebaseError(error, 'combining progress data');
            throw error;
        }
    },

    async refreshUserProgress(userId) {
        try {
            const user = getCurrentUser();
            if (!user) {
                throw new Error('User not authenticated');
            ...
            handleFirebaseError(error, 'refreshing user progress');
            throw error;
        }
    },

    async getBasicStatsFromFirestore() {
        const user = getCurrentUser();
        if (!user) {
            throw new Error('User not authenticated');
        }
    ...
    async getMonthlyProgressFromFirestore() {
        const user = getCurrentUser();
        if (!user) {
            throw new Error('User not authenticated');
        }
    ...
    async getRecentStudyTimeFromFirestore() {
        const user = getCurrentUser();
        if (!user) {
            throw new Error('User not authenticated');
        }
